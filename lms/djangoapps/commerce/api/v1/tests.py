...
from commerce.api.v1.serializers import DATETIME_FORMAT
...
class CourseApiViewTestMixin(object):
    ...
    def setUp(self):
        ...
        self.course_mode = CourseMode.objects.create(course_id=self.course.id, mode_slug=u'verified', min_price=100,
                                                     currency=u'USD', sku=u'ABC123', expiration_datetime=timezone.now())

    @staticmethod
    def _serialize_course_mode(course_mode):
        """ Serialize a CourseMode to a dict. """

        expires = course_mode.expiration_datetime.strftime(DATETIME_FORMAT) if course_mode.expiration_datetime else None
        return {
